...
export class BufferPublisher {
  ...
  async createPost({
    ...
    // Blindaje Anti-Duplicados: Verificar si el texto o la imagen ya se enviaron recientemente a Buffer
    try {
      let feed = [];
      try {
        const { getSocialFeedCloudState } = await import('./cloud-state.js');
        feed = await getSocialFeedCloudState();
      } catch (_) {}

      if (!Array.isArray(feed) || feed.length === 0) {
        const fs = await import('fs');
        const path = await import('path');
        const feedFile = path.resolve('social_published_feed.json');
      ...
      if (Array.isArray(feed) && feed.length > 0) {
        const now = Date.now();
        const incomingImgUrls = (assets || []).map(a => a?.image?.url).filter(Boolean);

        const duplicate = feed.find(entry => {
          const hoursAgo = (now - new Date(entry.timestamp).getTime()) / (1000 * 60 * 60);
          if (hoursAgo > 24) return false;
          const chanRes = entry.results && entry.results[targetService];
          if (!chanRes || !chanRes.success) return false;

          // Verificar si el texto es idéntico o muy similar
          const textMatch = (entry.theme && text.includes(entry.theme.substring(0, 30))) ||
                            (entry.textSnippet && text.substring(0, 50) === entry.textSnippet);

          // Verificar si la misma imagen ya fue publicada en las últimas 24h
          const imgMatch = incomingImgUrls.length > 0 && entry.imageUrls &&
                           incomingImgUrls.some(url => entry.imageUrls.includes(url));

          return textMatch || imgMatch;
        });

        if (duplicate) {
          console.warn(`🛡️ [Buffer Anti-Duplicado] Publicación OMITIDA para ${targetService}: contenido o imagen ya despachados recientemente.`);
          return { id: duplicate.results[targetService]?.id || 'DEDUP_SKIPPED', skipped: true };
        }
      }
    } catch (dedupErr) {
      // Continuar si la bitácora no está disponible
    }
