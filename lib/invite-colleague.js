...
import { escapeHtml, checkRateLimit, setStrictCors } from './security.js';
...
async function dispatchEmail({ to, subject, html, replyTo }) {
  ...
  if (gmailUser && gmailPass && !gmailUser.includes('tu_correo')) {
    try {
      ...
      console.warn('[Invite] Gmail SMTP limit/error, cayendo a Ethereal fallback:', gmailErr.message);
    }
  }
...
export default async function handler(req, res) {
  ...
  if (!checkRateLimit('invite_colleague_' + clientIp, 5, 3600000)) {
    return res.status(429).json({ error: 'Límite de invitaciones excedido para esta dirección IP (máximo 5 por hora). Intente más tarde.' });
  }
