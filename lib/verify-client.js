...
function getSecret() {
  return process.env.SUPABASE_SERVICE_ROLE_KEY || 'auditflow-fiduciary-token-secret-2026';
}

function generateSessionToken(email, plan) {
  const secret = getSecret();
  const expiresAt = Date.now() + 30 * 24 * 60 * 60 * 1000; // 30 días
  const planName = plan || 'Enterprise';
  const payload = `${email}|${planName}|${expiresAt}`;
