...
import { verifyAdminAuth, escapeHtml, setStrictCors } from './security.js';

// CORREGIDO (2026-09-17) — CRASH CONFIRMADO EN PRODUCCIÓN:
// este archivo importaba estáticamente '../api/outreach.js', que NO existe
// en este paquete (SCRIPTS_CLOUD_AUDIFLOW.zip solo incluye scripts/, lib/ y
// .github/workflows/). Un import estático a un módulo inexistente hace que
// Node lance ERR_MODULE_NOT_FOUND al cargar el archivo, lo cual rompía por
// completo el cron de Waalaxy: scripts/dispatch_waalaxy_medianos_campaign.js
// (invocado por .github/workflows/cron_waalaxy_sprint_48h.yml y por la
// opción "waalaxy_sprint" del despacho manual) importa este archivo, así
// que la campaña fallaba el 100% de las veces que se ejecutaba.
// Se cambia a import dinámico con fallback seguro para que el módulo cargue
// sin crashear. IMPORTANTE: esto NO reemplaza los datos reales — solo evita
// el crash. Debes restaurar el archivo real '../api/outreach.js' (con los
// leads reales) en el proyecto completo para que estas campañas vuelvan a
// tener datos que enviar.
let generateLegalExecutiveLeads = () => [];
let NORDIC_LEGAL_EXECUTIVE_LEADS = [];
let DACH_LEGAL_EXECUTIVE_LEADS = [];
try {
  const outreachModule = await import('../api/outreach.js');
  generateLegalExecutiveLeads = outreachModule.generateLegalExecutiveLeads || generateLegalExecutiveLeads;
  NORDIC_LEGAL_EXECUTIVE_LEADS = outreachModule.NORDIC_LEGAL_EXECUTIVE_LEADS || [];
  DACH_LEGAL_EXECUTIVE_LEADS = outreachModule.DACH_LEGAL_EXECUTIVE_LEADS || [];
} catch (e) {
  console.error('[WaalaxySync] CRÍTICO: no se pudo cargar ../api/outreach.js — usando listas vacías para evitar un crash total. Restaura ese archivo en el proyecto completo. Detalle:', e.message);
}
...
export default async function handler(req, res) {
  setStrictCors(req, res, 'GET, POST, OPTIONS', 'Content-Type, Authorization, x-admin-password, x-waalaxy-token');
  ...
  if (req.method === 'POST') {
    try {
      ...
      if (action === 'dispatch_dach_campaign' || (action === 'dispatch_campaign' && (campaign === 'leads_dach_alemania' || campaign === 'dach'))) {
      ...
      if (action === 'dispatch_nordic_campaign' || action === 'dispatch_campaign' || (action && campaign === 'leads_zona_nordica')) {
        const dispatchedLeads = [];
