...
import { getDailyBufferSchedule } from './buffer-content-calendar.js';
...
export default async function handler(req, res) {
  ...
  // CORREGIDO (2026-09-17): eliminado el fallback hardcodeado 'AuditFlow2026!'
  // y la comparacion insegura con '==='. Ahora se exige ADMIN_PASSWORD real
  // configurada en el entorno y se compara en tiempo constante.
  ...
  if (req.method === 'GET' || req.query?.action === 'daily_cron_dispatch') {
    if (!isAuthorized) {
      return res.status(401).json({
        ...
        hint: 'Configure el header x-admin-password o el header x-vercel-cron.'
      });
    }
