...
export const CONFIG = Object.freeze({
  ...
  EMAIL: Object.freeze({
    ...
    // Remitente Autenticado SMTP (Fallback de Emergencia)
    SMTP_USER: (process.env.GMAIL_USER || 'rick28191@gmail.com').trim(),
    SMTP_PASS: (process.env.GMAIL_APP_PASSWORD || '').replace(/\s+/g, '').trim(),
    
    // Remitente Corporativo Oficial (Resend API con DKIM/SPF)
    RESEND_API_KEY: (process.env.RESEND_API_KEY || '').trim(),
    FROM_TRANSACTIONAL: '"Ricardo | AuditFlow AI" <ricardo@audiflowai.com>',
    FROM_SALES: '"Ricardo | AuditFlow AI" <ricardo@audiflowai.com>',
    FROM_OUTREACH: '"Directora de Marketing | AuditFlow AI" <cmvo@audiflowai.com>',
    FROM_REPORTS: '"Directora de Marketing | AuditFlow AI" <cmvo@audiflowai.com>',
    ...
    BOUNCE_ISOLATION_RECIPIENT: 'tendenciaiatufuturo@gmail.com', // Absorbe 100% de rebotes, personas inactivas o despachos cerrados
    PERSONAL_SHIELD_ACTIVE: true, // Bloqueo estricto: rick28191@gmail.com solo recibe compras confirmadas ($19, $69, $590) y reportes ejecutivos directos
  }),

  // 2. Seguridad y Autenticacion Administrativa
  // CORREGIDO (2026-09-17): sin fallback hardcodeado. Si ADMIN_PASSWORD no
  // esta configurada, queda vacia y validateSystemConfig() lanza error
  // fail-fast en vez de aceptar silenciosamente una contrasena publica.
  ...
  // 4. Pasarelas de Pago (REGLA FIDUCIARIA: Stripe NO opera nativamente en El Salvador)
  // - Para cobros en El Salvador / Bancos Locales: Wompi (Banco Agrícola / Grupo Bancolombia)
  // - Para liquidación instantánea global y local directa a Strike: Bitcoin Lightning Network (rick28@strike.me)
  // - Stripe: Solo para cuentas internacionales / entidades extranjeras registradas
  PAYMENTS: Object.freeze({
    LIGHTNING_ADDRESS: (process.env.LIGHTNING_ADDRESS || 'rick28@strike.me').trim(),
    STRIPE_SECRET_KEY: (process.env.STRIPE_SECRET_KEY || '').trim(),
    WOMPI_APP_ID: (process.env.WOMPI_APP_ID || '').trim(),
    WOMPI_API_KEY: (process.env.WOMPI_API_KEY || '').trim(),
    WOMPI_LINK_19: (process.env.WOMPI_LINK_19 || 'https://s.wompi.sv/2199099xEg').trim(),
  }),
  ...
  // 6. Canales de Contacto Directo y Atención VIP (SSOT)
  CONTACT: Object.freeze({
    WHATSAPP_NUMBER: (process.env.WHATSAPP_NUMBER || '50375743444').trim(),
    WHATSAPP_FORMATTED: '+503 7574 3444',
    WHATSAPP_LINK: 'https://wa.me/50375743444',
  })
});
