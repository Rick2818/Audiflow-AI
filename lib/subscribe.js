...
import { setStrictCors, verifyAdminAuth, checkRateLimit } from './security.js';
...
export async function sendSubscriptionWelcomeEmail({ to, name, interval = 'monthly', lang = 'es' }) {
  ...
  const html = isEn ? `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #0b0f19; color: #ffffff; padding: 30px; border-radius: 12px; max-width: 620px; margin: 0 auto; border: 1px solid #38bdf8;">
      ...
      <div style="background-color: #111827; border: 1px solid #374151; padding: 16px; border-radius: 10px; margin: 20px 0; font-size: 12px; color: #9ca3af;">
        ...
        <p style="margin: 4px 0;">&bull; WhatsApp VIP Concierge: <a href="https://wa.me/50375743444" style="color: #10b981; text-decoration: none;">+503 7574 3444</a></p>
  ...
  ` : `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #0b0f19; color: #ffffff; padding: 30px; border-radius: 12px; max-width: 620px; margin: 0 auto; border: 1px solid #38bdf8;">
      ...
      <div style="background-color: #111827; border: 1px solid #374151; padding: 16px; border-radius: 10px; margin: 20px 0; font-size: 12px; color: #9ca3af;">
        ...
        <p style="margin: 4px 0;">&bull; WhatsApp VIP Concierge: <a href="https://wa.me/50375743444" style="color: #10b981; text-decoration: none;">+503 7574 3444</a> (Atención directa con Dirección)</p>
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // CORREGIDO (2026-09-17): rate limit anti-abuso. El endpoint escribe en
    // Supabase, crea sesiones de Stripe y envía correo — sin límite era un
    // vector de spam/DoS de bajo costo para un atacante.
    ...
    const isTestMode = Boolean(body?.test_mode) || process.env.NODE_ENV === 'test';
    const rateCheck = checkRateLimit('subscribe_' + clientIp, isTestMode ? 100 : 15, 60000);
    if (!rateCheck.allowed) {
      return res.status(429).json({ error: 'Demasiadas solicitudes. Intente de nuevo en un momento.' });
    }

    // CORREGIDO (2026-09-17) — VULNERABILIDAD CRÍTICA DE BYPASS DE PAGO:
    // `body.test_mode` venía del cliente sin ninguna autenticación y, si era
    // truthy, el código de más abajo insertaba en Supabase una suscripción
    // con status:'active' y enviaba un "recibo oficial" de pago SETTLED.
    // Como verify-client.js concede sesión Enterprise a cualquier email con
    // una fila status:'active' en `subscriptions`, cualquier persona podía
    // autoactivarse acceso Enterprise gratis con solo:
    //   POST /api/subscribe { test_mode: true, email: "atacante@x.com" }
    // Ahora test_mode solo es válido si la petición trae credenciales de
    // administrador válidas (misma verificación que el resto del panel admin).
    const isAdminTestMode = Boolean(body.test_mode) && verifyAdminAuth(req);
    ...
    // Disparar Correo de Bienvenida Corporativa + Recibo B2B al cliente ÚNICAMENTE en modo de prueba explícito Y autenticado como admin
    // En producción, el recibo y la activación se procesan tras la confirmación fiduciaria del webhook
    if (isAdminTestMode) {
      await sendSubscriptionWelcomeEmail({ to: customerEmail, name: customerName, interval: planInterval, lang: lang || 'es' });
    }

    // Disparar Notificación al propietario únicamente en modo de prueba o confirmación de pago
    try {
      const ownerEmail = (process.env.PERSONAL_NOTIFICATION_EMAIL || CONFIG.EMAIL?.OWNER_SALES || 'rick28191@gmail.com').trim();
      const gmailUser = (process.env.GMAIL_USER || CONFIG.EMAIL?.SMTP_USER || '').trim();
      const gmailPass = (process.env.GMAIL_APP_PASSWORD || CONFIG.EMAIL?.SMTP_PASS || '').replace(/\s+/g, '').trim();

      if (!isAdminTestMode && gmailUser && gmailPass && !gmailUser.includes('tu_correo')) {
        const transporter = nodemailer.createTransport({
          service: 'gmail',
          auth: { user: gmailUser, pass: gmailPass }
        });
    ...
    // Persistencia fiduciaria en Supabase (status: 'pending' hasta confirmación del webhook)
    if (supabase) {
      try {
        await supabase.from('subscriptions').insert([
          {
            ...
            price_usd: priceUsd,
            status: isAdminTestMode ? 'active' : 'pending',
            customer_email: customerEmail
          }
        ]);
      } catch (e) {
    ...
    return res.status(200).json({
      success: true,
      pending: !isAdminTestMode,
      message: isAdminTestMode
        ? `Suscripción Corporativa (${planInterval === 'annual' ? '$590/año' : '$69/mes'}) activada en modo prueba. Recibo oficial enviado a ${customerEmail}.`
        : `Solicitud de Suscripción Corporativa (${planInterval === 'annual' ? '$590/año' : '$69/mes'}) registrada como pendiente de liquidación.`,
      checkoutUrl: `${appUrl}/?status=${isAdminTestMode ? 'success_subscription' : 'pending_subscription'}&email=${encodeURIComponent(customerEmail)}`
    });
