...
import { setStrictCors } from './security.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Disparar Correo de Bienvenida Corporativa + Recibo B2B al cliente ÚNICAMENTE en modo de prueba explícito
        ...
        await supabase.from('subscriptions').insert([
          {
            ...
            price_usd: priceUsd,
            status: body.test_mode ? 'active' : 'pending',
            customer_email: customerEmail
          }
        ]);
      } catch (e) {
    ...
    return res.status(200).json({
      success: true,
      pending: !body.test_mode,
      message: body.test_mode
      ...
      checkoutUrl: `${appUrl}/?status=${body.test_mode ? 'success_subscription' : 'pending_subscription'}&email=${encodeURIComponent(customerEmail)}`
    });
