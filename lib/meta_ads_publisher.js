...
export const B2B_FIDUCIARY_TARGETING_CENTROAMERICA_MEXICO = {
  countries: ['SV', 'GT', 'CR', 'PA', 'MX'],
  ageMin: 28,
  ageMax: 64,
  workPositions: [
...
export function buildTargeting({
  ...
  aplicarFiltroFiduciario = true
}) {
  if (customTargeting) {
    return customTargeting;
  }

  const countries = Array.from(new Set([paisCodigo, ...paisesAdicionales]));
...
export async function publicarAnuncioMeta(params) {
  const {
    ...
    excludedCustomAudiences = [],
    customTargeting = null,
    estadoCampana = process.env.META_DEFAULT_AD_STATUS || 'ACTIVE',
    estadoAdSet = process.env.META_DEFAULT_AD_STATUS || 'ACTIVE',
