...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const lead of targetLeads) {
      ...
      const subject = `revisión contratos y redlines / ${company}`;
      const emailHtml = `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; font-size: 15px; color: #111827; line-height: 1.6; max-width: 580px;">
          ...
          <p style="background-color: #f8fafc; padding: 12px; border-left: 3px solid #2563eb; margin: 16px 0; font-size: 14px; border-radius: 4px;">
            ...
            <strong>• Opción 2:</strong> O si estás revisando algún borrador esta semana, puedes probarlo directamente en memoria RAM (sin guardar archivos) aquí: <a href="${appUrl}/?ref=waalaxy" style="color: #2563eb; text-decoration: none; font-weight: bold;">audiflowai.com →</a>
          </p>
