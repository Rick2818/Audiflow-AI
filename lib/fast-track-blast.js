...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { limit = 50, test_mode = false } = body;
    const allLeads = generateOutreachProspects('pareto_top20');
    // Tomar los 50 leads con la puntuación fiduciaria más alta (Score 99-98)
    ...
    for (const lead of targetLeads) {
      ...
      const subject = `revisión contratos y facturas / ${company}`;
      const emailHtml = `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; font-size: 15px; color: #111827; line-height: 1.6; max-width: 580px;">
          ...
          <p>Veo que lideras la gestión financiera / legal en <strong>${escapeHtml(company)}</strong>.</p>
          <p>Desarrollamos <strong>AuditFlow AI</strong> (<a href="${appUrl}" style="color: #2563eb; text-decoration: none;">audiflowai.com</a>), una herramienta que audita contratos de proveedores y facturas en 10 segundos antes de la firma, detectando penalizaciones ocultas y sobrecostos por indexación (ahorro promedio: $3,500 - $12,000 USD por contrato).</p>
          <p>¿Te hace sentido que te envíe un resumen de 1 página con los puntos de fuga más frecuentes que estamos detectando en el sector, o te pillo en mal momento?</p>
