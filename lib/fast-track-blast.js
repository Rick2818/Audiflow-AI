...
// CORREGIDO (2026-09-17) — mismo problema que en lib/waalaxy-sync.js: import
// estático a '../api/outreach.js', archivo ausente en este paquete, que
// crasheaba el módulo entero con ERR_MODULE_NOT_FOUND. Se usa import
// dinámico con fallback seguro. Restaura el archivo real del proyecto
// completo para recuperar los leads reales de prospección.
let generateOutreachProspects = () => [];
try {
  const outreachModule = await import('../api/outreach.js');
  generateOutreachProspects = outreachModule.generateOutreachProspects || generateOutreachProspects;
} catch (e) {
  console.error('[FastTrack] CRÍTICO: no se pudo cargar ../api/outreach.js — usando lista vacía para evitar un crash total. Restaura ese archivo en el proyecto completo. Detalle:', e.message);
}
...
async function sendFastTrackEmail({ to, subject, html, replyTo }) {
  ...
  // CORREGIDO (2026-09-17): el fallback a Ethereal de abajo era código
  // muerto (inalcanzable) porque el "return" anterior siempre se ejecutaba
  // primero. El resultado real era que, si Resend fallaba o no estaba
  // configurado, la función SIEMPRE reportaba "éxito simulado" sin que se
  // enviara correo real y sin ninguna alternativa. Esto podía ocultar
  // fallos de envío reales en campañas de prospección. Se documenta el
  // comportamiento intencional (nunca usar Gmail personal para campañas
  // masivas) y se elimina el código muerto que nunca se ejecutaba.
  console.warn('[FastTrack] Resend no configurado o fallido. Simulando envío seguro para evitar rebotes en Gmail personal.');
  return { success: false, provider: 'simulated_protection', warning: 'Correo NO enviado: Resend no disponible y no hay fallback real configurado para campañas masivas.' };
}

export default async function handler(req, res) {
  setStrictCors(req, res, 'POST, OPTIONS', 'Content-Type, Authorization, x-admin-password');
  ...
  try {
    ...
    for (const lead of targetLeads) {
      ...
      if (!test_mode) {
        ...
        // CORREGIDO (2026-09-17): antes se marcaba 'sent' incondicionalmente
        // aunque el envío hubiera fallado o sido simulado.
        results.push({ email, name, company, status: dispatchRes.success ? 'sent' : 'failed', provider: dispatchRes.provider });
      } else {
