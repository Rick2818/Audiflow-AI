...
import { escapeHtml, checkRateLimit } from './security.js';
...
async function sendAuditReportEmail({ to, subject, html }) {
  // Prioridad 1 y ÚNICA: Resend SDK con dominio verificado audiflowai.com (Cero uso de Gmail personal)
  if (resendClient) {
    try {
      const res = await resendClient.emails.send({
        from: 'AuditFlow AI <ricardo@audiflowai.com>',
        to: [to],
  ...
  return { success: false, error: 'Resend API no configurado' };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { name: rawName, email, document_name, audit_data, lang } = body;
    const auditData = audit_data || {};

    if (!rawName || !email || !String(email).includes('@')) {
      return res.status(400).json({ error: 'Nombre y correo electrónico válido son requeridos.' });
    }

    // CORREGIDO (2026-09-17): rate limit anti-abuso. Este endpoint envía
    // correo HTML real desde el dominio verificado de la empresa; sin límite
    // de tasa podía usarse como relay masivo de phishing/spam.
    ...
    const rateCheck = checkRateLimit('lead_capture_' + clientIp, 10, 3600000);
    if (!rateCheck.allowed) {
      return res.status(429).json({ error: 'Demasiadas solicitudes de auditoría desde esta IP. Intente más tarde.' });
    }

    // CORREGIDO (2026-09-17): todo campo controlado por el cliente que se
    // inserta en el HTML del correo (nombre, nombre de documento, tipo,
    // hallazgos) se escapa para evitar inyección HTML / phishing usando el
    // dominio verificado de la empresa como remitente.
    const rawDocName = document_name || 'Contrato_Comercial.pdf';
    const rawDocType = audit_data?.document_type || 'Contrato de Servicios Comercial';
    const reportId = 'rep_' + Math.random().toString(36).substring(2, 11);
    const leadScore = Math.min(100, Math.max(0, parseInt(audit_data?.lead_score, 10) || 88));
    const isEnterpriseCandidate = leadScore >= 75;
    const leakageVal = Number(audit_data?.total_financial_leakage) || 14850;
    const tags = generateLeadTags({ document_type: rawDocType, lead_score: leadScore, leakage_usd: leakageVal });

    // Valores escapados: SOLO para interpolar en el HTML del correo, nunca para BD.
    const name = escapeHtml(rawName);
    const docName = escapeHtml(rawDocName);
    const docType = escapeHtml(rawDocType);

    console.log(`📩 [LEAD CAPTURED] ${rawName} <${email}> - Doc: ${rawDocName}`);

    // Persistencia en Supabase (datos crudos, sin escapar, para no corromper la BD)
    if (supabase) {
      try {
    ...
    findings.forEach((item, idx) => {
      // CORREGIDO (2026-09-17): campos de "findings" provienen del cliente
      // (audit_data.findings) y se insertaban sin escapar en el HTML del
      // correo enviado desde el dominio verificado — vector de inyección
      // HTML / phishing. Se escapan todos los campos de texto libre.
      const sevRaw = item.severity || (idx === 0 ? 'CRITICAL' : (idx === 1 ? 'HIGH' : 'MEDIUM'));
      const sevColor = sevRaw.includes('CRIT') || sevRaw.includes('KRIT') ? '#ef4444' : (sevRaw.includes('HIGH') || sevRaw.includes('HOCH') || sevRaw.includes('ALT') ? '#f59e0b' : '#38bdf8');
      const sev = escapeHtml(sevRaw);
      const clause = escapeHtml(item.clause_reference || `Cláusula #${idx + 1}`);
      const title = escapeHtml(item.title || `Hallazgo #${idx + 1}`);
      const impact = Number(item.financial_impact) || Math.round(leakageVal / 3);
      const solution = escapeHtml(item.actionable_solution || item.solution || defaultFindings[idx]?.actionable_solution || 'Renegociar y topar penalizaciones contractuales.');
