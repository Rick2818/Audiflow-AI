...
export class N8nAgentBridge {
  constructor() {
    ...
    this.adminEmail = 'tendenciaiatufuturo@gmail.com';
  ...
  async triggerN8nWebhook(endpointPath, payload) {
    ...
    try {
      ...
      if (!res.ok) {
        return { success: true, mode: 'local_simulation', status: res.status, endpoint: endpointPath, payload };
      }
      return { success: true, mode: 'live_n8n', status: res.status, endpoint: endpointPath, payload };
    } catch (err) {
