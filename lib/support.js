...
import { setStrictCors, escapeHtml, checkRateLimit } from './security.js';
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #0b0f19; color: #ffffff; padding: 25px; border-radius: 12px; max-width: 600px; margin: 0 auto; border: 1px solid #38bdf8;">
      ...
      <div style="background-color: #111827; border: 1px solid #334155; padding: 18px; border-radius: 8px; margin: 20px 0; font-size: 13px;">
        <p style="margin: 4px 0;"><strong>Remitente:</strong> <span style="color: #38bdf8;">${escapeHtml(email) || 'No proporcionado'}</span></p>
        <p style="margin: 4px 0;"><strong>ID de Reporte (si aplica):</strong> ${escapeHtml(reportId) || 'N/A (Consulta General)'}</p>
        <p style="margin: 4px 0;"><strong>Idioma:</strong> ${escapeHtml(lang) || 'es'}</p>
...
${escapeHtml(message) || 'Sin mensaje'}
        ...
        <p style="margin: 8px 0 0 0; color: #64748b; font-size: 11px;">User Agent: ${escapeHtml(userAgent) || 'N/A'}</p>
      </div>
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    // CORREGIDO (2026-09-17): sin rate limiting este endpoint permitía
    // inundar Supabase (system_issues) y disparar correos ilimitados.
    ...
    if (!checkRateLimit('support_' + clientIp, 10, 60000).allowed) {
      return res.status(429).json({ success: false, error: 'Demasiadas solicitudes de soporte. Intente en un momento.' });
    }
    try {
      let body = req.body || {};
      ...
      return res.status(200).json({
        ...
        whatsapp: CONFIG.CONTACT?.WHATSAPP_FORMATTED || '+503 7574 3444'
      });
