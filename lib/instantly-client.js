...
export class InstantlyClient {
  ...
  async getWarmupHealth(email = 'ricardo.audiflowai@gmail.com') {
    ...
    const accounts = Array.isArray(accountsRes) 
      ? accountsRes 
      : (accountsRes?.items || accountsRes?.data || []);
    ...
    // Instantly V2: warmup_status: 1 (activo), 0 (inactivo); status: 1 (active)
    const isWarmupActive = target.warmup_status === 1 || target.warmup_status === '1' || target.warmup_status === 'active' || target.warmup_status === 'enabled';
    const isAccountActive = target.status === 1 || target.status === '1' || target.status === 'active';
    const healthScore = target.stat_warmup_score ?? target.health_score ?? target.warmup?.score ?? 0;
    const isReadyForCold = healthScore >= 90;

    return {
      found: true,
      email,
      active: isWarmupActive && isAccountActive,
      accountStatus: target.status,
      warmupStatus: target.warmup_status,
      healthScore,
      isReadyForCold,
      dailyLimit: target.daily_limit || 30,
