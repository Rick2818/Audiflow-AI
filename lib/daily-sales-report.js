...
export async function generateAndSendDailySalesReport({ timeSlot = '6:00 PM' } = {}) {
  ...
  if (gmailUser && gmailPass) {
    try {
      ...
      // 1. CANAL PRIMARIO Y SEGURO: Resend API con firmas DKIM y SPF oficiales (@audiflowai.com)
      if (resendKey) {
        try {
      ...
      // 2. FALLBACK DE EMERGENCIA: Gmail SMTP Relay (solo si Resend no estuviera disponible)
      if (!deliverySuccess && gmailUser && gmailPass) {
        try {
          ...
          console.error(`❌ [SALES REPORT SMTP FALLBACK ERROR]:`, smtpErr.message);
          emailError = smtpErr.message;
        }
      }
    } catch (outerErr) {
      console.error('❌ [SALES REPORT GENERAL ERROR]:', outerErr);
      emailError = outerErr.message;
    }
  }
