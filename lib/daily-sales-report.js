...
export async function generateAndSendDailySalesReport({ timeSlot = '6:00 PM' } = {}) {
  ...
  const primaryRecipients = [CONFIG.EMAIL.OWNER_SALES, CONFIG.EMAIL.OWNER_CONTROL];
        ...
        <div style="padding: 24px;">
          ...
          <div style="background-color: #064e3b; border: 1px solid #10b981; padding: 16px; border-radius: 8px; margin-bottom: 20px;">
            ...
            <ul style="margin: 0; padding-left: 18px; font-size: 13px; color: #ecfdf5; line-height: 1.7;">
              ...
              <li><strong>2. Seguimiento Fiduciario Activo:</strong> Monitorear interacciones de CFOs y socios directores que revisan cláusulas y descargan redlines en Word.</li>
  ...
  // 1. CANAL PRIMARIO: Resend API con firmas DKIM y SPF (@audiflowai.com)
  if (resendKey) {
    try {
      ...
      if (resendResult?.data?.id) {
        console.log(`✅ [SALES REPORT DISPATCHED VIA RESEND DKIM] Corte ${timeSlot} enviado con éxito:`, resendResult.data.id);
        emailSent = true;
      } else {
        throw new Error(resendResult?.error?.message || 'Error en respuesta de Resend');
      }
    } catch (resendErr) {
      console.warn(`⚠️ [SALES REPORT RESEND NOTICE - ACTIVANDO FALLBACK SMTP]:`, resendErr.message);
      emailError = `Resend: ${resendErr.message}`;
    }
  }

  // 2. CANAL SECUNDARIO: Gmail SMTP Relay (si Resend no envió o agotó cuota)
  if (!emailSent && gmailUser && gmailPass) {
    try {
      ...
      console.log(`✅ [SALES REPORT DISPATCHED VIA SMTP FALLBACK] Corte ${timeSlot}:`, sendResult.messageId);
      emailSent = true;
      emailError = null;
    } catch (smtpErr) {
      console.warn(`⚠️ [SALES REPORT SMTP FALLBACK NOTICE]:`, smtpErr.message);
      emailError = (emailError ? `${emailError} | ` : '') + `SMTP: ${smtpErr.message}`;
    }
  }

  // Si no se pudo despachar por ningún canal de correo (ej. cuota excedida en la nube)
  if (!emailSent) {
    console.log(`ℹ️ [SALES REPORT TELEMETRY] Reporte financiero generado y registrado en bitácora (Balance: $${grossRevenueToday.toFixed(2)} USD).`);
  }
