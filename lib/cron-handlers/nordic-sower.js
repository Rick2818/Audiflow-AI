...
import { getCloudState, setCloudState } from '../cloud-state.js';
import { NORDIC_MIDMARKET_PARTNERS } from '../../scripts/nordic_midmarket_daily_sower.mjs';
import { verifyAdminAuth } from '../security.js';
import { getLegalNoticeForOutbound, resolveJurisdiction, getTripwirePrice } from '../legal-jurisdictions.js';
...
export default async function handler(req, res) {
  ...
  for (const partner of todaysBatch) {
    ...
    const trialUrl = `https://audiflowai.com/?ref=nordic-cron&lang=en&country=${jur.id}&lead=${encodeURIComponent(partner.firstName)}`;
    const subject = `[Case Brief] 45 pages reviewed, but 18 words in Schedule C cost ${localDamage} / ${partner.firm}`;
    const legalNotice = getLegalNoticeForOutbound(partner, 'nordic');
    const html = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-size: 14px; color: #1e293b; max-width: 580px; line-height: 1.65; margin: 0 auto; background-color: #ffffff; padding: 26px; border: 1px solid #e2e8f0; border-radius: 8px;">
        <div style="border-bottom: 1px solid #e2e8f0; padding-bottom: 12px; margin-bottom: 18px;">
          ...
          <span style="color: #64748b; font-size: 12px; margin-left: 10px;">| EU Fiduciary Contracts Engine (${jur.countryNameEn})</span>
        </div>
        <p>Dear Partner ${partner.firstName},</p>
        <p>At <strong>${partner.firm}</strong> (${partner.city}), managing complex commercial vendor agreements without exposing mid-market clients to uncapped liabilities or unfavorable terms against <strong>${jur.standardContracts}</strong> is a continuous challenge.</p>
        <p>AuditFlow AI runs 100% in volatile RAM strictly under <strong>EU GDPR Art. 28</strong>: zero permanent storage, audit in 8.2s and direct Word (.docx with Track Changes) generation for just <strong>${tripwire}</strong>.</p>
        ${legalNotice}
