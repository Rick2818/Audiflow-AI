...
import { getSocialFeedCloudState, saveSocialFeedCloudState } from '../cloud-state.js';
...
export default async function handler(req, res) {
  ...
  if (!isAuthorized) {
    return res.status(401).json({
      success: false,
      error: 'No autorizado. Se requiere autorización de Vercel Cron o token admin (x-admin-password).'
    });
  }
