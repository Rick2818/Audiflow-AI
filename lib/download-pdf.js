export default async function handler(req, res) {
  ...
  try {
    ...
    const certCode = forensic_cert || ('AUDITFLOW-SHA256-' + (forensic_hash ? forensic_hash.substring(0, 16).toUpperCase() : 'VERIFIED'));
    const hashCode = forensic_hash || 'sha256_ephemeral_ram_' + Date.now();
