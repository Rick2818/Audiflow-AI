...
app.all('/api/verify-client', verifyClientHandler);
...
// PROMPT OFICIAL JSON DE GEMINI 2.5 FLASH (ADAPTADO A EL SALVADOR Y CENTROAMÉRICA)
...
// ==============================================================================
// ENDPOINT 1: POST /api/audit (ANÁLISIS MULTIMODAL EN MEMORIA VOLÁTIL CON OCR)
// ==============================================================================
  ...
  try {
    ...
    if (req.body && req.body.document_base64) {
      ...
      if (isPdf) {
        ...
        parts.push({
          text: `${GEMINI_SYSTEM_PROMPT}\n\nAnaliza este documento PDF mercantil (nombre: ${fileName}, postura: ${partyStance}) bajo las leyes comerciales de El Salvador y Centroamérica.`
        });
...
CLÁUSULA 4: INDEXACIÓN DOBLE. Los honorarios se reajustarán semestralmente conforme al IPC más un 5% adicional acumulativo retroactivo.`;
      ...
      parts.push({
        text: `${GEMINI_SYSTEM_PROMPT}\n\nDOCUMENTO A AUDITAR (Nombre: ${fileName}, Postura: ${partyStance}):\n${extractedText}`
      });
    }

    // Validación fiduciaria obligatoria de GEMINI_API_KEY (CERO MOCKS)
    ...
    if (!apiKey || apiKey === 'tu_gemini_api_key_aqui') {
      return res.status(503).json({
        ...
        action_required: 'Configure GEMINI_API_KEY en el archivo .env.'
      });
    }
    ...
    if (!geminiRes.ok && (geminiRes.status === 404 || geminiRes.status === 400)) {
      ...
      if (fallbackRes.ok) geminiRes = fallbackRes;
    }
      ...
      let bodyHtml = `
        <div style="font-family: Arial, sans-serif; background-color: #0b0f19; color: #ffffff; padding: 28px; border-radius: 12px; border: 1px solid #10b981; max-width: 600px; margin: 0 auto;">
          ...
          <p style="text-align: center; margin: 25px 0;">
            <a href="https://audiflowai.com/?ref=outreach_gift_${encodeURIComponent(country)}" style="background-color: #10b981; color: #000000; font-weight: bold; font-size: 15px; padding: 14px 28px; border-radius: 8px; text-decoration: none; display: inline-block;">🎁 Iniciar Auditoría Gratuita para ${company}</a>
          </p>
      ...
      if (isEn) {
        ...
        bodyHtml = `
          <div style="font-family: Arial, sans-serif; background-color: #0b0f19; color: #ffffff; padding: 28px; border-radius: 12px; border: 1px solid #10b981; max-width: 600px; margin: 0 auto;">
            ...
            <p style="text-align: center; margin: 25px 0;">
              <a href="https://audiflowai.com/?ref=outreach_gift_en_${encodeURIComponent(country)}" style="background-color: #10b981; color: #000000; font-weight: bold; font-size: 15px; padding: 14px 28px; border-radius: 8px; text-decoration: none; display: inline-block;">🎁 Start Free Audit for ${company}</a>
            </p>
            <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #1f2937;">
